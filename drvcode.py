...
#df.columns
col_num = dbutils.fs.head("/mnt/my-data/%s/col_num.csv" % (uid),)
data_col = dbutils.fs.head("/mnt/my-data/%s/data.csv" % (uid),)
...
for colname in data_col:
  if ((colname not in  featureColumns) and (colname != target_col)):
        df = df.drop(colname)
